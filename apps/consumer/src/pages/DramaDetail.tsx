  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-[#030303] flex flex-col items-center justify-center text-zinc-300 px-6 text-center">
        <p className="text-sm font-semibold">Gagal memuat drama</p>
        <div className="mt-4 flex gap-2">
          ...
          <button
            type="button"
            onClick={goBack}
            className="rounded-full border border-zinc-800 px-4 py-2 text-xs font-bold text-zinc-300"
          >
            Kembali
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#030303] text-zinc-100 pb-10">
      ...
      <div className="relative w-full h-[260px] bg-zinc-950 overflow-hidden">
        ...
        <div className="absolute inset-0 filter blur-xl opacity-30 scale-110">
          <img src={posterSrc(drama.posterUrl)} alt="" className="w-full h-full object-cover" />
        </div>
        ...
        <div className="absolute top-4 left-4 z-20">
          <button
            ...
            onClick={goBack}
            aria-label="Kembali"
            className="w-11 h-11 rounded-full bg-black/55 backdrop-blur-md border border-zinc-800 flex items-center justify-center text-white hover:bg-black/80 transition-colors"
          >
        ...
        <div className="absolute bottom-4 left-0 w-full px-4 z-20 flex gap-4 items-end">
          <div className="w-24 shrink-0 aspect-[2/3] rounded-lg overflow-hidden border border-zinc-800 shadow-xl bg-zinc-900">
            <img src={posterSrc(drama.posterUrl)} alt={drama.title} className="w-full h-full object-cover" />
          </div>
          <div className="flex-1 pb-1">
            <div className="flex items-center gap-1.5 mb-1">
              {drama.country ? (
                <span className="px-1.5 py-0.5 rounded text-[10px] font-extrabold bg-rose-500/20 text-rose-400 border border-rose-500/30 uppercase">
                  {drama.country}
                </span>
              ) : null}
              {drama.rating ? (
                <span className="px-1.5 py-0.5 rounded text-[10px] font-extrabold bg-amber-500/20 text-amber-400 border border-amber-500/30 flex items-center gap-0.5">
                  ★ {drama.rating.toFixed(1)}
                </span>
              ) : null}
            </div>
            <h1 className="text-lg font-extrabold text-white leading-tight line-clamp-2">{drama.title}</h1>
            <p className="text-[11px] text-zinc-400 mt-1">
              {[drama.year, `${episodes.length} Episode`].filter(Boolean).join(" • ")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 mt-4">
        {episodes.length === 0 || !startEpisode ? (
          ...
          <Link
            to={`/drama/${drama.slug}/episode/${startEpisode}`}
      ...
      <div className="px-4 mt-6 border-b border-zinc-900">
        <div className="flex gap-6" role="tablist" aria-label="Detail drama">
          <button
            type="button"
            role="tab"
            id="tab-episodes"
            aria-controls="panel-episodes"
      ...
      <div className="px-4 mt-4">
        {activeTab === "info" ? (
          <div id="panel-info" role="tabpanel" aria-labelledby="tab-info" className="space-y-4 animate-fadeIn">
            {drama.synopsis && (
              <div>
                <h3 className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Sinopsis</h3>
            ...
            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-zinc-900/60">
              {drama.genres && drama.genres.length > 0 && (
                <div>
                  <h4 className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Genre</h4>
              ...
              {drama.country && (
                <div>
                  <h4 className="text-[11px] font-bold text-zinc-500 uppercase tracking-wider">Asal</h4>
          ...
          <div id="panel-episodes" role="tabpanel" aria-labelledby="tab-episodes" className="animate-fadeIn">
            {episodes.length === 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      {isVip ? (
                        ...
                        <span className="absolute bottom-1 text-[10px] tracking-wide font-extrabold text-zinc-500 uppercase">
                          FREE
                        </span>
                      )}
                    </Link>
